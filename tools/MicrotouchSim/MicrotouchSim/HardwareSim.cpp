...
//======================================================================================================
//	File system
//	Clumsy reconstruction of FAT info from Win32 apis...
...
int SimFAT_Directory(DirectoryProc directoryProc, u8* buffer, void* ref)
{
	...
	do
	{
		...
		{
			DirectoryEntry e = {0};
			e.length = ffd.nFileSizeLow;
			if (ffd.cAlternateFileName[0])
			{
				memcpy(&e.fatname[0],&ffd.cAlternateFileName[0],8);
				memcpy(&e.fatname[8],&ffd.cAlternateFileName[9],3);
			} else {
				ToFATName(&e.fatname[0],ffd.cFileName);
			}
			e.attributes = ffd.dwFileAttributes;
			directoryProc(&e,i++,ref);
		}
