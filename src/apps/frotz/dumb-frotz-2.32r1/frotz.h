...
void SetZByte(long a, zbyte v);
zbyte GetZByte(long a);
...
#define SET_BYTE(addr,v)  { SetZByte(addr,v);		}
#define LOW_BYTE(addr,v)  { v = GetZByte(addr);		}

#define SET_WORD(addr,v)  { SetZByte(addr,hi(v)); SetZByte(addr+1,lo(v)); }
#define LOW_WORD(addr,v)  { v = GetZWord(addr); }
#define HIGH_WORD(addr,v) { v = GetZWord(addr); }	// <<32 bit TODO

#define CODE_BYTE(v)	  { v = GetCByte(); }
#define CODE_WORD(v)      { v = GetCWord(); }
...
#define POP()			(*GetStack(sp++))
#define GETSTACK(_x)	(*GetStack(_x))
#define PUSH(_x)		(*GetStackW(--sp) = (zword)(_x))
#define ADDSTACK(_x,_v) (*GetStackW(_x) += (_v))
#define SETSTACK(_x,_v) (*GetStackW(_x) = (_v))

#define runtime_error(_x) deadmeat()
#endif
